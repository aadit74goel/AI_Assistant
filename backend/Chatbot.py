...
import datetime  # Importing the datetime module for real-time date and time handling.
...
client = Groq(api_key="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")
...
try:
    with open(r"Data\ChatLog.json", "r") as f:
        messages = load(f)  # Load existing messages from file.
...
def AnswerModifier(Answer):
    ...
    modified_answer = '\n'.join(non_empty_lines)  # Join the cleaned lines back.
...
def ChatBot(Query):
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            messages=[{"role": "system", "content": RealtimeInformation()}] + messages,  # Include system message.
